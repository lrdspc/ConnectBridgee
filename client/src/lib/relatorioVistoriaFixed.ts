/**
 * Gerador SIMPLIFICADO de Relatórios DOCX com formatação ABNT para documentos formais
 * 
 * Características:
 * - Formatação Times New Roman 12pt
 * - Espaçamento entre linhas 1,5
 * - Margens ABNT
 * - Formatação precisa para documentos técnicos/jurídicos
 */
...
function log(...args: any[]) {
  if (DEBUG) console.log("[RelatorioFixed]", ...args);
}
 ...
 */
export async function gerarRelatorioFixo(relatorio: RelatorioVistoria): Promise<Blob> {
  try {
    log("Iniciando geração de relatório com formatação ABNT");
