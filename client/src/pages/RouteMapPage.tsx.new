  ...
  return (
    <PageTransition>
      <div className="page min-h-screen bg-gray-50">
        ...
        {/* Status de localização e controles */}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className={`w-full ${!isMobile ? 'desktop-tabs' : ''}`}>
          ...
          <TabsContent value="lista" className="mt-0 px-4 pt-2 pb-24">
            ...
            {mapVisits.length === 0 ? (
              ...
              <div className={`space-y-3 ${!isMobile ? 'visits-grid' : ''}`}>
                {mapVisits.map((visit, index) => (
                  <Card 
                    key={visit.id}
                    className={`hover:border-primary transition-colors ${!isMobile ? 'desktop-card' : ''} ${selectedVisitId === visit.id ? 'border-primary' : ''}`}
