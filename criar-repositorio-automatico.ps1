...
Set-Location "c:\Users\lrdsp\Desktop\ReplitExport-lrdspc\ConnectBridge"
...
if ($ghPath) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Criar repositório
        Write-Host "📦 Criando repositório 'connectbridge'..." -ForegroundColor Yellow
        & $ghPath.Source repo create connectbridge --description "Sistema de Relatórios de Vistoria Técnica para telhas Brasilit/Saint-Gobain" --public --source=. --remote=origin --push
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "🎉 REPOSITÓRIO CRIADO COM SUCESSO!" -ForegroundColor Green
            Write-Host ""
            Write-Host "📋 RESUMO:" -ForegroundColor Cyan
            Write-Host "- ✅ Repositório criado: https://github.com/$(& $ghPath.Source auth status --show-token 2>$null | Select-String 'Logged in to github.com as (.+)' | ForEach-Object {$_.Matches[0].Groups[1].Value})/connectbridge"
            ...
            Start-Process "https://github.com/$(& $ghPath.Source auth status 2>$null | Select-String 'Logged in to github.com as (.+)' | ForEach-Object {$_.Matches[0].Groups[1].Value})/connectbridge"
        } else {
            Write-Host "❌ Erro ao criar repositório. Tentando método manual..." -ForegroundColor Red
            & $ghPath.Source repo create connectbridge --description "Sistema de Relatórios de Vistoria Técnica" --public
            
            # Adicionar origin e fazer push manual
            git remote add origin "https://github.com/$(& $ghPath.Source auth status 2>$null | Select-String 'Logged in to github.com as (.+)' | ForEach-Object {$_.Matches[0].Groups[1].Value})/connectbridge.git"
            git branch -M main
    ...
    Write-Host "1. Nome: connectbridge"
